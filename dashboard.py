...
def plot_weather_impact(filtered_df):
    # Menghitung rata-rata 'total_rentals' berdasarkan 'weathersit'
    avg_cnt = filtered_df.groupby('weathersit', as_index=False)['total_rentals'].mean()

    plt.figure(figsize=(10, 6))
    palette = sns.color_palette("Set1")
    sns.barplot(x='weathersit', y='total_rentals', data=avg_cnt, errorbar=None, palette=palette)
...
def plot_monthly_rentals(filtered_df):
    ...
    monthly_rentals = filtered_df.groupby('mnth')['total_rentals'].sum()
...
def plot_hourly_rentals(filtered_df):
    plt.figure(figsize=(10, 6))
    hourly_rentals = filtered_df.groupby('hr')['total_rentals'].mean()
